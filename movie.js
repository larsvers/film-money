...
const noDecimal = d3.format('.2s');
const singleDecimal = d3.format('.2s');
const translateG = function (x, y) {
  return `translate(${x}, ${y})`;
};
const hashToObjects = function (_) {
  const keys = d3.keys(_);
  const values = d3.values(_);
...
const getAxisLabel = function (name) {
  return d3
    .select('.y.axis')
    .selectAll('g > text')
    .filter(function (d) {
...
const globals = (function () {
  ...
  config.windowSizeFlag = !!(window.innerWidth < 750 || window.innerHeight < 500);
...
d3.csv('data/movies_ia.csv', type, function (data) {
  ...
  data = _.sortBy(data, function (el) {
    return el.production_budget;
  }); // Sort before feeding into any function. Otherwise the data won't be sorted descendingly but ascendingly

  handler.pressed(undefined, '#start_value');
  handler.plotpoint.initial(data);
  setTimeout(function () {
    config.pageload = false;
  }, 2000); // pageload flag makes sure this isn't getting fired from the scrol listener as well
  ...
  const listener = (function () {
    ...
    d3.selectAll('button.value').on('mousedown', function () {
      ...
      x === 'start_value' || x === 'production_budget'
        ? (config.baseline = false)
    ...
    $('#intro').on('itemfocus', function (event, item) {
      ...
      const action = isNaN(item.data.action)
        ? item.data.action.split(',').map(Number)
        : [item.data.action]; // convert single number or number of strings to array
...
var handler = (function () {
  ...
  my.pressed = function (that, value) {
    ...
    if ($(that).hasClass('rating')) {
      ...
      const a = config.extentZ.slice();
      const b = a.filter(function (el) {
        return el !== that.id;
      }); // find the non-pressed id
    ...
    if (
      d3.select(that).classed('rating') === false &&
      d3.select(that).classed('scatterplot') === false
    ) {
  ...
  my.legend = function (genreFlag) {
    ...
    const legendRatingSvgWidth = $('#legend-ratings svg').width();
    const legendScatterImgWidth = $('#legend-scatter img').width();

    if (config.rating) {
      // show legend
      d3.select('#legend-ratings')
        .transition()
        .style('width', `${legendRatingSvgWidth}px`)
        .transition()
        .delay(300)
        .style('opacity', 1);
    } else {
    ...
    if (config.scatterplot) {
      ...
      d3.select('#legend-scatter')
        .transition()
        .style('width', `${legendScatterImgWidth}px`)
        .transition()
        .delay(300)
        .style('opacity', 1);
    } else {
  ...
  my.plotpoint.compose = function (data) {
    ...
    d3.select('div#container').datum(data).call(newChart);
  }; // set new category
...
function chart() {
  ...
  function my(selection) {
    selection.each(function (data, i) {
      ...
      dataNest = _.sortBy(dataNest, function (el) {
        return el[sortBy];
      });

      // ===  Measure extents === //

      const objX = measureVars(extentX, varX);
      const objY = measureVars(extentY, varY);
      const objZ = measureVars(extentZ, varZ);
      ...
      function measureVars(varsExtent, variable) {
        // error handling
        try {
          if (varsExtent.constructor !== Array)
            throw 'Pls input an array of variable(s) in the chart composition';
        } catch (err) {
          console.error(err);
        }
        ...
        const varsStr = [];
        varsExtent.map(function (el) {
          varsStr.push(String(el));
        });
      ...
      scaleY = d3
        .scalePoint()
        .domain(
          dataNest.map(function (el) {
            return el[objY.value];
          })
        )
        .range([height, 0])
        .padding(1);
      ...
      if (scatterplot) {
        scaleY = d3.scaleLinear().domain(objY.extent).range([height, 0]);
      }

      scaleZ = d3
        .scaleSqrt()
        .domain([objZ.extent[0], objZ.extent[1]])
        .range(config.windowSizeFlag ? [2, 10] : [3, 20]); // range smaller for small screens
      ...
      const legendBuilder = (function () {
        ...
        const steps = 10;
        const delta = (objZ.extent[1] - objZ.extent[0]) / steps;
        const newExtent = [objZ.extent[0], objZ.extent[1] + delta];
        const range = d3.range(newExtent[0], newExtent[1], delta);
        ...
        const svgL = d3.select('#legend-ratings').selectAll('svg.svgL').data([dataLegend]); // data join with a single data element

        const svgEnterL = svgL.enter().append('svg'); // manifestation of enter (only happens the first time within a load period)

        // d3.select('#legend > svg')
        svgEnterL.classed('svgL', true).attr('width', 120).attr('height', 30); // give the svg attributes

        // --- Create circles and text --- //

        const form = d3.format('.2n');

        const gL = d3
          .select('svg.svgL') // this M.U.S.T. be a d3.select-selector and can't be the stored variable (here: svgL).
          .selectAll('g.gl')
        ...
        const gEnterL = gL
          ...
          .attr('transform', function (d, i) {
            return translateG((100 / dataLegend.length) * i + 10, 30);
          });
      ...
      const svg = d3.select(this).selectAll('svg').data([dataNest]);

      const gEnter = svg.enter().append('svg').classed('svgMain', true).append('g');
      ...
      axisX = d3
        ...
        .tickFormat(function (d) {
          return scatterplot
            ? singleDecimal(d)
            : noDecimal(d).replace('M', ' mil').replace('G', ' bil');
        })
        .tickSize(-height)
        .tickPadding(10);
      ...
      d3.selectAll('g.x.axis > g.tick > text').style('text-anchor', 'start');
      ...
      if (scatterplot) {
        ...
        d3.selectAll('.y.axis text').style('fill', '#BABAB1');
      }
      ...
      if (!scatterplot) {
        ...
        lines
          ...
          .style('stroke', function (d) {
            return rating ? scaleZCol(d[objZ.value]) : '#ccc';
          })
          .transition()
          .duration(dur)
          .delay(function (d, i) {
        ...
        // exit
        lines.exit().transition().duration(dur).style('opacity', 1e-6).remove();
      } // no lines for any scatterplot

      // --- Circles --- //

      const radius = config.windowSizeFlag ? 2 : 5; // circle radius smaller for small screens
      ...
      circles
        ...
        .attr('r', function (d) {
          return rating ? scaleZ(d[objZ.value]) : radius;
        })
        .style('fill', function (d) {
          return rating ? scaleZCol(d[objZ.value]) : '#ccc';
        });
      ...
      // exit
      circles.exit().transition().duration(dur).style('opacity', 1e-6).remove();

      // === Specific actions and interactions === //

      // --- Baseline circles --- //

      const baselineCircles = (function () {
        if (baseline && d3.select('.baseline').empty()) {
          d3.select('g.lollipops')
            .selectAll('circle.baseline')
            .data(
              dataNest.map(function (el) {
                return { yValues: el[objY.value], xValues: el[config.compareTo] };
              })
            )
            .enter()
            .append('circle')
            ...
            .attr('cy', function (d) {
              return scaleY(d.yValues);
            })
            .attr('cx', function (d) {
              return scaleX(d.xValues);
            })
            .attr('r', radius)
            .style('fill', '#4161F0')
            .style('opacity', 1e-6);
      ...
      const scatterplotProperties = (function () {
        if (scatterplot) {
          // line removal
      ...
      const genreHighlight = (function () {
        // --- Data prep for the genre highlight --- //

        const genres = _.uniq(
          data.map(function (el) {
            return el.genre_main;
          })
        ); // unique list of genres
        const arrAxis = data
          .filter(function (el) {
            return el.category === config.keyValue;
          })
          .map(function (el) {
            return el.movie;
          }); // unique list of films
        ...
        genreArray = _.sortBy(genreArray, function (el) {
          return el.genre;
        });
        ...
        // Data join
        const genreButtons = d3.select('#genreMenu').selectAll('.genres').data(genreArray);
        ...
        genreButtons
          ...
          .style('color', function (d) {
            return d.filmlist.length > 0 ? '#D9D9CE' : '#777';
          });

        // Update buttons
        genreButtons.style('color', function (d) {
          return d.filmlist.length > 0 ? '#D9D9CE' : '#777';
        });
        ...
        genreButtons.on('mouseover', function (d) {
          ...
          const arrAxis = d3.select('.y.axis').selectAll('g > text').data();
          const intersection = _.intersection(arrAxis, d.filmlist);

          intersection.forEach(function (el, i) {
            const axisLabel = getAxisLabel(el);
      ...
      const tooltip = (function () {
        d3.selectAll('.circles').on('mouseover', function (d) {
          function format(num) {
            return `$ ${singleDecimal(num).replace('M', ' mil').replace('G', ' bil')}`;
          }

          const html =
            `${'<div id="ttWrap">' + '<div id="pic">' + '<img src="images/films/'}${
              d.image
            }" alt="Avatar">` +
            `</div>` +
            `<div id="text">` +
            ...
            `<p class="tooltipText">Worldwide revenue: ${format(d.worldwide_gross)}</p>${
              d.rating_imdb === 0
                ? '<p class="tooltipText break">IMDb doesn\'t rate this film</p>'
                : `<p class="tooltipText break">IMDb rating: ${d.rating_imdb} of 10</p>`
            }${
              d.rating_rt === 0
                ? '<p class="tooltipText break">Rotten Tomatoes doesn\'t rate this film</p>'
                : `<p class="tooltipText break">Rotten Tomatoes rating: ${d.rating_rt} of 10</p>`
            }</div>` +
            `</div>`;
